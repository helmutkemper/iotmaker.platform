package factoryColorNames

import "image/color"

func NewLemonchiffon() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xfa, B: 0xcd, A: 0xff} // rgb(255, 250, 205)
}

func NewLightblue() color.RGBA {
	return color.RGBA{R: 0xad, G: 0xd8, B: 0xe6, A: 0xff} // rgb(173, 216, 230)
}

func NewLightcoral() color.RGBA {
	return color.RGBA{R: 0xf0, G: 0x80, B: 0x80, A: 0xff} // rgb(240, 128, 128)
}

func NewLightcyan() color.RGBA {
	return color.RGBA{R: 0xe0, G: 0xff, B: 0xff, A: 0xff} // rgb(224, 255, 255)
}

func NewLightgoldenrodyellow() color.RGBA {
	return color.RGBA{R: 0xfa, G: 0xfa, B: 0xd2, A: 0xff} // rgb(250, 250, 210)
}

func NewLightgray() color.RGBA {
	return color.RGBA{R: 0xd3, G: 0xd3, B: 0xd3, A: 0xff} // rgb(211, 211, 211)
}

func NewLightgreen() color.RGBA {
	return color.RGBA{R: 0x90, G: 0xee, B: 0x90, A: 0xff} // rgb(144, 238, 144)
}

func NewLightgrey() color.RGBA {
	return color.RGBA{R: 0xd3, G: 0xd3, B: 0xd3, A: 0xff} // rgb(211, 211, 211)
}

func NewLightpink() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xb6, B: 0xc1, A: 0xff} // rgb(255, 182, 193)
}

func NewLightsalmon() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xa0, B: 0x7a, A: 0xff} // rgb(255, 160, 122)
}

func NewLightseagreen() color.RGBA {
	return color.RGBA{R: 0x20, G: 0xb2, B: 0xaa, A: 0xff} // rgb(32, 178, 170)
}

func NewLightskyblue() color.RGBA {
	return color.RGBA{R: 0x87, G: 0xce, B: 0xfa, A: 0xff} // rgb(135, 206, 250)
}

func NewLightslategray() color.RGBA {
	return color.RGBA{R: 0x77, G: 0x88, B: 0x99, A: 0xff} // rgb(119, 136, 153)
}

func NewLightslategrey() color.RGBA {
	return color.RGBA{R: 0x77, G: 0x88, B: 0x99, A: 0xff} // rgb(119, 136, 153)
}

func NewLightsteelblue() color.RGBA {
	return color.RGBA{R: 0xb0, G: 0xc4, B: 0xde, A: 0xff} // rgb(176, 196, 222)
}

func NewLightyellow() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xff, B: 0xe0, A: 0xff} // rgb(255, 255, 224)
}

func NewLime() color.RGBA {
	return color.RGBA{R: 0x00, G: 0xff, B: 0x00, A: 0xff} // rgb(0, 255, 0)
}

func NewLimegreen() color.RGBA {
	return color.RGBA{R: 0x32, G: 0xcd, B: 0x32, A: 0xff} // rgb(50, 205, 50)
}

func NewLinen() color.RGBA {
	return color.RGBA{R: 0xfa, G: 0xf0, B: 0xe6, A: 0xff} // rgb(250, 240, 230)
}

func NewMagenta() color.RGBA {
	return color.RGBA{R: 0xff, G: 0x00, B: 0xff, A: 0xff} // rgb(255, 0, 255)
}

func NewMaroon() color.RGBA {
	return color.RGBA{R: 0x80, G: 0x00, B: 0x00, A: 0xff} // rgb(128, 0, 0)
}

func NewMediumaquamarine() color.RGBA {
	return color.RGBA{R: 0x66, G: 0xcd, B: 0xaa, A: 0xff} // rgb(102, 205, 170)
}

func NewMediumblue() color.RGBA {
	return color.RGBA{R: 0x00, G: 0x00, B: 0xcd, A: 0xff} // rgb(0, 0, 205)
}

func NewMediumorchid() color.RGBA {
	return color.RGBA{R: 0xba, G: 0x55, B: 0xd3, A: 0xff} // rgb(186, 85, 211)
}

func NewMediumpurple() color.RGBA {
	return color.RGBA{R: 0x93, G: 0x70, B: 0xdb, A: 0xff} // rgb(147, 112, 219)
}

func NewMediumseagreen() color.RGBA {
	return color.RGBA{R: 0x3c, G: 0xb3, B: 0x71, A: 0xff} // rgb(60, 179, 113)
}

func NewMediumslateblue() color.RGBA {
	return color.RGBA{R: 0x7b, G: 0x68, B: 0xee, A: 0xff} // rgb(123, 104, 238)
}

func NewMediumspringgreen() color.RGBA {
	return color.RGBA{R: 0x00, G: 0xfa, B: 0x9a, A: 0xff} // rgb(0, 250, 154)
}

func NewMediumturquoise() color.RGBA {
	return color.RGBA{R: 0x48, G: 0xd1, B: 0xcc, A: 0xff} // rgb(72, 209, 204)
}

func NewMediumvioletred() color.RGBA {
	return color.RGBA{R: 0xc7, G: 0x15, B: 0x85, A: 0xff} // rgb(199, 21, 133)
}

func NewMidnightblue() color.RGBA {
	return color.RGBA{R: 0x19, G: 0x19, B: 0x70, A: 0xff} // rgb(25, 25, 112)
}

func NewMintcream() color.RGBA {
	return color.RGBA{R: 0xf5, G: 0xff, B: 0xfa, A: 0xff} // rgb(245, 255, 250)
}

func NewMistyrose() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xe4, B: 0xe1, A: 0xff} // rgb(255, 228, 225)
}

func NewMoccasin() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xe4, B: 0xb5, A: 0xff} // rgb(255, 228, 181)
}

func NewNavajowhite() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xde, B: 0xad, A: 0xff} // rgb(255, 222, 173)
}

func NewNavy() color.RGBA {
	return color.RGBA{R: 0x00, G: 0x00, B: 0x80, A: 0xff} // rgb(0, 0, 128)
}

func NewOldlace() color.RGBA {
	return color.RGBA{R: 0xfd, G: 0xf5, B: 0xe6, A: 0xff} // rgb(253, 245, 230)
}

func NewOlive() color.RGBA {
	return color.RGBA{R: 0x80, G: 0x80, B: 0x00, A: 0xff} // rgb(128, 128, 0)
}

func NewOlivedrab() color.RGBA {
	return color.RGBA{R: 0x6b, G: 0x8e, B: 0x23, A: 0xff} // rgb(107, 142, 35)
}

func NewOrange() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xa5, B: 0x00, A: 0xff} // rgb(255, 165, 0)
}

func NewOrangered() color.RGBA {
	return color.RGBA{R: 0xff, G: 0x45, B: 0x00, A: 0xff} // rgb(255, 69, 0)
}

func NewOrchid() color.RGBA {
	return color.RGBA{R: 0xda, G: 0x70, B: 0xd6, A: 0xff} // rgb(218, 112, 214)
}

func NewPalegoldenrod() color.RGBA {
	return color.RGBA{R: 0xee, G: 0xe8, B: 0xaa, A: 0xff} // rgb(238, 232, 170)
}

func NewPalegreen() color.RGBA {
	return color.RGBA{R: 0x98, G: 0xfb, B: 0x98, A: 0xff} // rgb(152, 251, 152)
}

func NewPaleturquoise() color.RGBA {
	return color.RGBA{R: 0xaf, G: 0xee, B: 0xee, A: 0xff} // rgb(175, 238, 238)
}

func NewPalevioletred() color.RGBA {
	return color.RGBA{R: 0xdb, G: 0x70, B: 0x93, A: 0xff} // rgb(219, 112, 147)
}

func NewPapayawhip() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xef, B: 0xd5, A: 0xff} // rgb(255, 239, 213)
}

func NewPeachpuff() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xda, B: 0xb9, A: 0xff} // rgb(255, 218, 185)
}

func NewPeru() color.RGBA {
	return color.RGBA{R: 0xcd, G: 0x85, B: 0x3f, A: 0xff} // rgb(205, 133, 63)
}

func NewPink() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xc0, B: 0xcb, A: 0xff} // rgb(255, 192, 203)
}

func NewPlum() color.RGBA {
	return color.RGBA{R: 0xdd, G: 0xa0, B: 0xdd, A: 0xff} // rgb(221, 160, 221)
}

func NewPowderblue() color.RGBA {
	return color.RGBA{R: 0xb0, G: 0xe0, B: 0xe6, A: 0xff} // rgb(176, 224, 230)
}

func NewPurple() color.RGBA {
	return color.RGBA{R: 0x80, G: 0x00, B: 0x80, A: 0xff} // rgb(128, 0, 128)
}

func NewRed() color.RGBA {
	return color.RGBA{R: 0xff, G: 0x00, B: 0x00, A: 0xff} // rgb(255, 0, 0)
}

func NewRosybrown() color.RGBA {
	return color.RGBA{R: 0xbc, G: 0x8f, B: 0x8f, A: 0xff} // rgb(188, 143, 143)
}

func NewRoyalblue() color.RGBA {
	return color.RGBA{R: 0x41, G: 0x69, B: 0xe1, A: 0xff} // rgb(65, 105, 225)
}

func NewSaddlebrown() color.RGBA {
	return color.RGBA{R: 0x8b, G: 0x45, B: 0x13, A: 0xff} // rgb(139, 69, 19)
}

func NewSalmon() color.RGBA {
	return color.RGBA{R: 0xfa, G: 0x80, B: 0x72, A: 0xff} // rgb(250, 128, 114)
}

func NewSandybrown() color.RGBA {
	return color.RGBA{R: 0xf4, G: 0xa4, B: 0x60, A: 0xff} // rgb(244, 164, 96)
}

func NewSeagreen() color.RGBA {
	return color.RGBA{R: 0x2e, G: 0x8b, B: 0x57, A: 0xff} // rgb(46, 139, 87)
}

func NewSeashell() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xf5, B: 0xee, A: 0xff} // rgb(255, 245, 238)
}

func NewSienna() color.RGBA {
	return color.RGBA{R: 0xa0, G: 0x52, B: 0x2d, A: 0xff} // rgb(160, 82, 45)
}

func NewSilver() color.RGBA {
	return color.RGBA{R: 0xc0, G: 0xc0, B: 0xc0, A: 0xff} // rgb(192, 192, 192)
}

func NewSkyblue() color.RGBA {
	return color.RGBA{R: 0x87, G: 0xce, B: 0xeb, A: 0xff} // rgb(135, 206, 235)
}

func NewSlateblue() color.RGBA {
	return color.RGBA{R: 0x6a, G: 0x5a, B: 0xcd, A: 0xff} // rgb(106, 90, 205)
}

func NewSlategray() color.RGBA {
	return color.RGBA{R: 0x70, G: 0x80, B: 0x90, A: 0xff} // rgb(112, 128, 144)
}

func NewSlategrey() color.RGBA {
	return color.RGBA{R: 0x70, G: 0x80, B: 0x90, A: 0xff} // rgb(112, 128, 144)
}

func NewSnow() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xfa, B: 0xfa, A: 0xff} // rgb(255, 250, 250)
}

func NewSpringgreen() color.RGBA {
	return color.RGBA{R: 0x00, G: 0xff, B: 0x7f, A: 0xff} // rgb(0, 255, 127)
}

func NewSteelblue() color.RGBA {
	return color.RGBA{R: 0x46, G: 0x82, B: 0xb4, A: 0xff} // rgb(70, 130, 180)
}

func NewTan() color.RGBA {
	return color.RGBA{R: 0xd2, G: 0xb4, B: 0x8c, A: 0xff} // rgb(210, 180, 140)
}

func NewTeal() color.RGBA {
	return color.RGBA{R: 0x00, G: 0x80, B: 0x80, A: 0xff} // rgb(0, 128, 128)
}

func NewThistle() color.RGBA {
	return color.RGBA{R: 0xd8, G: 0xbf, B: 0xd8, A: 0xff} // rgb(216, 191, 216)
}

func NewTomato() color.RGBA {
	return color.RGBA{R: 0xff, G: 0x63, B: 0x47, A: 0xff} // rgb(255, 99, 71)
}

func NewTurquoise() color.RGBA {
	return color.RGBA{R: 0x40, G: 0xe0, B: 0xd0, A: 0xff} // rgb(64, 224, 208)
}

func NewViolet() color.RGBA {
	return color.RGBA{R: 0xee, G: 0x82, B: 0xee, A: 0xff} // rgb(238, 130, 238)
}

func NewWheat() color.RGBA {
	return color.RGBA{R: 0xf5, G: 0xde, B: 0xb3, A: 0xff} // rgb(245, 222, 179)
}

func NewWhite() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xff, B: 0xff, A: 0xff} // rgb(255, 255, 255)
}

func NewWhitesmoke() color.RGBA {
	return color.RGBA{R: 0xf5, G: 0xf5, B: 0xf5, A: 0xff} // rgb(245, 245, 245)
}

func NewYellow() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xff, B: 0x00, A: 0xff} // rgb(255, 255, 0)
}

func NewYellowgreen() color.RGBA {
	return color.RGBA{R: 0x9a, G: 0xcd, B: 0x32, A: 0xff} // rgb(154, 205, 50)
}

func NewAliceblueTransparent() color.RGBA {
	return color.RGBA{R: 0xf0, G: 0xf8, B: 0xff, A: 0x00} // rgb(240, 248, 255)
}

func NewAntiquewhiteTransparent() color.RGBA {
	return color.RGBA{R: 0xfa, G: 0xeb, B: 0xd7, A: 0x00} // rgb(250, 235, 215)
}

func NewAquaTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0xff, B: 0xff, A: 0x00} // rgb(0, 255, 255)
}

func NewAquamarineTransparent() color.RGBA {
	return color.RGBA{R: 0x7f, G: 0xff, B: 0xd4, A: 0x00} // rgb(127, 255, 212)
}

func NewAzureTransparent() color.RGBA {
	return color.RGBA{R: 0xf0, G: 0xff, B: 0xff, A: 0x00} // rgb(240, 255, 255)
}

func NewBeigeTransparent() color.RGBA {
	return color.RGBA{R: 0xf5, G: 0xf5, B: 0xdc, A: 0x00} // rgb(245, 245, 220)
}

func NewBisqueTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xe4, B: 0xc4, A: 0x00} // rgb(255, 228, 196)
}

func NewBlackTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0x00, B: 0x00, A: 0x00} // rgb(0, 0, 0)
}

func NewBlanchedalmondTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xeb, B: 0xcd, A: 0x00} // rgb(255, 235, 205)
}

func NewBlueTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0x00, B: 0xff, A: 0x00} // rgb(0, 0, 255)
}

func NewBluevioletTransparent() color.RGBA {
	return color.RGBA{R: 0x8a, G: 0x2b, B: 0xe2, A: 0x00} // rgb(138, 43, 226)
}

func NewBrownTransparent() color.RGBA {
	return color.RGBA{R: 0xa5, G: 0x2a, B: 0x2a, A: 0x00} // rgb(165, 42, 42)
}

func NewBurlywoodTransparent() color.RGBA {
	return color.RGBA{R: 0xde, G: 0xb8, B: 0x87, A: 0x00} // rgb(222, 184, 135)
}

func NewCadetblueTransparent() color.RGBA {
	return color.RGBA{R: 0x5f, G: 0x9e, B: 0xa0, A: 0x00} // rgb(95, 158, 160)
}

func NewChartreuseTransparent() color.RGBA {
	return color.RGBA{R: 0x7f, G: 0xff, B: 0x00, A: 0x00} // rgb(127, 255, 0)
}

func NewChocolateTransparent() color.RGBA {
	return color.RGBA{R: 0xd2, G: 0x69, B: 0x1e, A: 0x00} // rgb(210, 105, 30)
}

func NewCoralTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0x7f, B: 0x50, A: 0x00} // rgb(255, 127, 80)
}

func NewCornflowerblueTransparent() color.RGBA {
	return color.RGBA{R: 0x64, G: 0x95, B: 0xed, A: 0x00} // rgb(100, 149, 237)
}

func NewCornsilkTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xf8, B: 0xdc, A: 0x00} // rgb(255, 248, 220)
}

func NewCrimsonTransparent() color.RGBA {
	return color.RGBA{R: 0xdc, G: 0x14, B: 0x3c, A: 0x00} // rgb(220, 20, 60)
}

func NewCyanTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0xff, B: 0xff, A: 0x00} // rgb(0, 255, 255)
}

func NewDarkblueTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0x00, B: 0x8b, A: 0x00} // rgb(0, 0, 139)
}

func NewDarkcyanTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0x8b, B: 0x8b, A: 0x00} // rgb(0, 139, 139)
}

func NewDarkgoldenrodTransparent() color.RGBA {
	return color.RGBA{R: 0xb8, G: 0x86, B: 0x0b, A: 0x00} // rgb(184, 134, 11)
}

func NewDarkgrayTransparent() color.RGBA {
	return color.RGBA{R: 0xa9, G: 0xa9, B: 0xa9, A: 0x00} // rgb(169, 169, 169)
}

func NewDarkgreenTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0x64, B: 0x00, A: 0x00} // rgb(0, 100, 0)
}

func NewDarkgreyTransparent() color.RGBA {
	return color.RGBA{R: 0xa9, G: 0xa9, B: 0xa9, A: 0x00} // rgb(169, 169, 169)
}

func NewDarkkhakiTransparent() color.RGBA {
	return color.RGBA{R: 0xbd, G: 0xb7, B: 0x6b, A: 0x00} // rgb(189, 183, 107)
}

func NewDarkmagentaTransparent() color.RGBA {
	return color.RGBA{R: 0x8b, G: 0x00, B: 0x8b, A: 0x00} // rgb(139, 0, 139)
}

func NewDarkolivegreenTransparent() color.RGBA {
	return color.RGBA{R: 0x55, G: 0x6b, B: 0x2f, A: 0x00} // rgb(85, 107, 47)
}

func NewDarkorangeTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0x8c, B: 0x00, A: 0x00} // rgb(255, 140, 0)
}

func NewDarkorchidTransparent() color.RGBA {
	return color.RGBA{R: 0x99, G: 0x32, B: 0xcc, A: 0x00} // rgb(153, 50, 204)
}

func NewDarkredTransparent() color.RGBA {
	return color.RGBA{R: 0x8b, G: 0x00, B: 0x00, A: 0x00} // rgb(139, 0, 0)
}

func NewDarksalmonTransparent() color.RGBA {
	return color.RGBA{R: 0xe9, G: 0x96, B: 0x7a, A: 0x00} // rgb(233, 150, 122)
}

func NewDarkseagreenTransparent() color.RGBA {
	return color.RGBA{R: 0x8f, G: 0xbc, B: 0x8f, A: 0x00} // rgb(143, 188, 143)
}

func NewDarkslateblueTransparent() color.RGBA {
	return color.RGBA{R: 0x48, G: 0x3d, B: 0x8b, A: 0x00} // rgb(72, 61, 139)
}

func NewDarkslategrayTransparent() color.RGBA {
	return color.RGBA{R: 0x2f, G: 0x4f, B: 0x4f, A: 0x00} // rgb(47, 79, 79)
}

func NewDarkslategreyTransparent() color.RGBA {
	return color.RGBA{R: 0x2f, G: 0x4f, B: 0x4f, A: 0x00} // rgb(47, 79, 79)
}

func NewDarkturquoiseTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0xce, B: 0xd1, A: 0x00} // rgb(0, 206, 209)
}

func NewDarkvioletTransparent() color.RGBA {
	return color.RGBA{R: 0x94, G: 0x00, B: 0xd3, A: 0x00} // rgb(148, 0, 211)
}

func NewDeeppinkTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0x14, B: 0x93, A: 0x00} // rgb(255, 20, 147)
}

func NewDeepskyblueTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0xbf, B: 0xff, A: 0x00} // rgb(0, 191, 255)
}

func NewDimgrayTransparent() color.RGBA {
	return color.RGBA{R: 0x69, G: 0x69, B: 0x69, A: 0x00} // rgb(105, 105, 105)
}

func NewDimgreyTransparent() color.RGBA {
	return color.RGBA{R: 0x69, G: 0x69, B: 0x69, A: 0x00} // rgb(105, 105, 105)
}

func NewDodgerblueTransparent() color.RGBA {
	return color.RGBA{R: 0x1e, G: 0x90, B: 0xff, A: 0x00} // rgb(30, 144, 255)
}

func NewFirebrickTransparent() color.RGBA {
	return color.RGBA{R: 0xb2, G: 0x22, B: 0x22, A: 0x00} // rgb(178, 34, 34)
}

func NewFloralwhiteTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xfa, B: 0xf0, A: 0x00} // rgb(255, 250, 240)
}

func NewForestgreenTransparent() color.RGBA {
	return color.RGBA{R: 0x22, G: 0x8b, B: 0x22, A: 0x00} // rgb(34, 139, 34)
}

func NewFuchsiaTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0x00, B: 0xff, A: 0x00} // rgb(255, 0, 255)
}

func NewGainsboroTransparent() color.RGBA {
	return color.RGBA{R: 0xdc, G: 0xdc, B: 0xdc, A: 0x00} // rgb(220, 220, 220)
}

func NewGhostwhiteTransparent() color.RGBA {
	return color.RGBA{R: 0xf8, G: 0xf8, B: 0xff, A: 0x00} // rgb(248, 248, 255)
}

func NewGoldTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xd7, B: 0x00, A: 0x00} // rgb(255, 215, 0)
}

func NewGoldenrodTransparent() color.RGBA {
	return color.RGBA{R: 0xda, G: 0xa5, B: 0x20, A: 0x00} // rgb(218, 165, 32)
}

func NewGrayTransparent() color.RGBA {
	return color.RGBA{R: 0x80, G: 0x80, B: 0x80, A: 0x00} // rgb(128, 128, 128)
}

func NewGreenTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0x80, B: 0x00, A: 0x00} // rgb(0, 128, 0)
}

func NewGreenyellowTransparent() color.RGBA {
	return color.RGBA{R: 0xad, G: 0xff, B: 0x2f, A: 0x00} // rgb(173, 255, 47)
}

func NewGreyTransparent() color.RGBA {
	return color.RGBA{R: 0x80, G: 0x80, B: 0x80, A: 0x00} // rgb(128, 128, 128)
}

func NewHoneydewTransparent() color.RGBA {
	return color.RGBA{R: 0xf0, G: 0xff, B: 0xf0, A: 0x00} // rgb(240, 255, 240)
}

func NewHotpinkTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0x69, B: 0xb4, A: 0x00} // rgb(255, 105, 180)
}

func NewIndianredTransparent() color.RGBA {
	return color.RGBA{R: 0xcd, G: 0x5c, B: 0x5c, A: 0x00} // rgb(205, 92, 92)
}

func NewIndigoTransparent() color.RGBA {
	return color.RGBA{R: 0x4b, G: 0x00, B: 0x82, A: 0x00} // rgb(75, 0, 130)
}

func NewIvoryTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xff, B: 0xf0, A: 0x00} // rgb(255, 255, 240)
}

func NewKhakiTransparent() color.RGBA {
	return color.RGBA{R: 0xf0, G: 0xe6, B: 0x8c, A: 0x00} // rgb(240, 230, 140)
}

func NewLavenderTransparent() color.RGBA {
	return color.RGBA{R: 0xe6, G: 0xe6, B: 0xfa, A: 0x00} // rgb(230, 230, 250)
}

func NewLavenderblushTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xf0, B: 0xf5, A: 0x00} // rgb(255, 240, 245)
}

func NewLawngreenTransparent() color.RGBA {
	return color.RGBA{R: 0x7c, G: 0xfc, B: 0x00, A: 0x00} // rgb(124, 252, 0)
}

func NewLemonchiffonTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xfa, B: 0xcd, A: 0x00} // rgb(255, 250, 205)
}

func NewLightblueTransparent() color.RGBA {
	return color.RGBA{R: 0xad, G: 0xd8, B: 0xe6, A: 0x00} // rgb(173, 216, 230)
}

func NewLightcoralTransparent() color.RGBA {
	return color.RGBA{R: 0xf0, G: 0x80, B: 0x80, A: 0x00} // rgb(240, 128, 128)
}

func NewLightcyanTransparent() color.RGBA {
	return color.RGBA{R: 0xe0, G: 0xff, B: 0xff, A: 0x00} // rgb(224, 255, 255)
}

func NewLightgoldenrodyellowTransparent() color.RGBA {
	return color.RGBA{R: 0xfa, G: 0xfa, B: 0xd2, A: 0x00} // rgb(250, 250, 210)
}

func NewLightgrayTransparent() color.RGBA {
	return color.RGBA{R: 0xd3, G: 0xd3, B: 0xd3, A: 0x00} // rgb(211, 211, 211)
}

func NewLightgreenTransparent() color.RGBA {
	return color.RGBA{R: 0x90, G: 0xee, B: 0x90, A: 0x00} // rgb(144, 238, 144)
}

func NewLightgreyTransparent() color.RGBA {
	return color.RGBA{R: 0xd3, G: 0xd3, B: 0xd3, A: 0x00} // rgb(211, 211, 211)
}

func NewLightpinkTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xb6, B: 0xc1, A: 0x00} // rgb(255, 182, 193)
}

func NewLightsalmonTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xa0, B: 0x7a, A: 0x00} // rgb(255, 160, 122)
}

func NewLightseagreenTransparent() color.RGBA {
	return color.RGBA{R: 0x20, G: 0xb2, B: 0xaa, A: 0x00} // rgb(32, 178, 170)
}

func NewLightskyblueTransparent() color.RGBA {
	return color.RGBA{R: 0x87, G: 0xce, B: 0xfa, A: 0x00} // rgb(135, 206, 250)
}

func NewLightslategrayTransparent() color.RGBA {
	return color.RGBA{R: 0x77, G: 0x88, B: 0x99, A: 0x00} // rgb(119, 136, 153)
}

func NewLightslategreyTransparent() color.RGBA {
	return color.RGBA{R: 0x77, G: 0x88, B: 0x99, A: 0x00} // rgb(119, 136, 153)
}

func NewLightsteelblueTransparent() color.RGBA {
	return color.RGBA{R: 0xb0, G: 0xc4, B: 0xde, A: 0x00} // rgb(176, 196, 222)
}

func NewLightyellowTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xff, B: 0xe0, A: 0x00} // rgb(255, 255, 224)
}

func NewLimeTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0xff, B: 0x00, A: 0x00} // rgb(0, 255, 0)
}

func NewLimegreenTransparent() color.RGBA {
	return color.RGBA{R: 0x32, G: 0xcd, B: 0x32, A: 0x00} // rgb(50, 205, 50)
}

func NewLinenTransparent() color.RGBA {
	return color.RGBA{R: 0xfa, G: 0xf0, B: 0xe6, A: 0x00} // rgb(250, 240, 230)
}

func NewMagentaTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0x00, B: 0xff, A: 0x00} // rgb(255, 0, 255)
}

func NewMaroonTransparent() color.RGBA {
	return color.RGBA{R: 0x80, G: 0x00, B: 0x00, A: 0x00} // rgb(128, 0, 0)
}

func NewMediumaquamarineTransparent() color.RGBA {
	return color.RGBA{R: 0x66, G: 0xcd, B: 0xaa, A: 0x00} // rgb(102, 205, 170)
}

func NewMediumblueTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0x00, B: 0xcd, A: 0x00} // rgb(0, 0, 205)
}

func NewMediumorchidTransparent() color.RGBA {
	return color.RGBA{R: 0xba, G: 0x55, B: 0xd3, A: 0x00} // rgb(186, 85, 211)
}

func NewMediumpurpleTransparent() color.RGBA {
	return color.RGBA{R: 0x93, G: 0x70, B: 0xdb, A: 0x00} // rgb(147, 112, 219)
}

func NewMediumseagreenTransparent() color.RGBA {
	return color.RGBA{R: 0x3c, G: 0xb3, B: 0x71, A: 0x00} // rgb(60, 179, 113)
}

func NewMediumslateblueTransparent() color.RGBA {
	return color.RGBA{R: 0x7b, G: 0x68, B: 0xee, A: 0x00} // rgb(123, 104, 238)
}

func NewMediumspringgreenTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0xfa, B: 0x9a, A: 0x00} // rgb(0, 250, 154)
}

func NewMediumturquoiseTransparent() color.RGBA {
	return color.RGBA{R: 0x48, G: 0xd1, B: 0xcc, A: 0x00} // rgb(72, 209, 204)
}

func NewMediumvioletredTransparent() color.RGBA {
	return color.RGBA{R: 0xc7, G: 0x15, B: 0x85, A: 0x00} // rgb(199, 21, 133)
}

func NewMidnightblueTransparent() color.RGBA {
	return color.RGBA{R: 0x19, G: 0x19, B: 0x70, A: 0x00} // rgb(25, 25, 112)
}

func NewMintcreamTransparent() color.RGBA {
	return color.RGBA{R: 0xf5, G: 0xff, B: 0xfa, A: 0x00} // rgb(245, 255, 250)
}

func NewMistyroseTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xe4, B: 0xe1, A: 0x00} // rgb(255, 228, 225)
}

func NewMoccasinTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xe4, B: 0xb5, A: 0x00} // rgb(255, 228, 181)
}

func NewNavajowhiteTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xde, B: 0xad, A: 0x00} // rgb(255, 222, 173)
}

func NewNavyTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0x00, B: 0x80, A: 0x00} // rgb(0, 0, 128)
}

func NewOldlaceTransparent() color.RGBA {
	return color.RGBA{R: 0xfd, G: 0xf5, B: 0xe6, A: 0x00} // rgb(253, 245, 230)
}

func NewOliveTransparent() color.RGBA {
	return color.RGBA{R: 0x80, G: 0x80, B: 0x00, A: 0x00} // rgb(128, 128, 0)
}

func NewOlivedrabTransparent() color.RGBA {
	return color.RGBA{R: 0x6b, G: 0x8e, B: 0x23, A: 0x00} // rgb(107, 142, 35)
}

func NewOrangeTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xa5, B: 0x00, A: 0x00} // rgb(255, 165, 0)
}

func NewOrangeredTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0x45, B: 0x00, A: 0x00} // rgb(255, 69, 0)
}

func NewOrchidTransparent() color.RGBA {
	return color.RGBA{R: 0xda, G: 0x70, B: 0xd6, A: 0x00} // rgb(218, 112, 214)
}

func NewPalegoldenrodTransparent() color.RGBA {
	return color.RGBA{R: 0xee, G: 0xe8, B: 0xaa, A: 0x00} // rgb(238, 232, 170)
}

func NewPalegreenTransparent() color.RGBA {
	return color.RGBA{R: 0x98, G: 0xfb, B: 0x98, A: 0x00} // rgb(152, 251, 152)
}

func NewPaleturquoiseTransparent() color.RGBA {
	return color.RGBA{R: 0xaf, G: 0xee, B: 0xee, A: 0x00} // rgb(175, 238, 238)
}

func NewPalevioletredTransparent() color.RGBA {
	return color.RGBA{R: 0xdb, G: 0x70, B: 0x93, A: 0x00} // rgb(219, 112, 147)
}

func NewPapayawhipTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xef, B: 0xd5, A: 0x00} // rgb(255, 239, 213)
}

func NewPeachpuffTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xda, B: 0xb9, A: 0x00} // rgb(255, 218, 185)
}

func NewPeruTransparent() color.RGBA {
	return color.RGBA{R: 0xcd, G: 0x85, B: 0x3f, A: 0x00} // rgb(205, 133, 63)
}

func NewPinkTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xc0, B: 0xcb, A: 0x00} // rgb(255, 192, 203)
}

func NewPlumTransparent() color.RGBA {
	return color.RGBA{R: 0xdd, G: 0xa0, B: 0xdd, A: 0x00} // rgb(221, 160, 221)
}

func NewPowderblueTransparent() color.RGBA {
	return color.RGBA{R: 0xb0, G: 0xe0, B: 0xe6, A: 0x00} // rgb(176, 224, 230)
}

func NewPurpleTransparent() color.RGBA {
	return color.RGBA{R: 0x80, G: 0x00, B: 0x80, A: 0x00} // rgb(128, 0, 128)
}

func NewRedTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0x00, B: 0x00, A: 0x00} // rgb(255, 0, 0)
}

func NewRosybrownTransparent() color.RGBA {
	return color.RGBA{R: 0xbc, G: 0x8f, B: 0x8f, A: 0x00} // rgb(188, 143, 143)
}

func NewRoyalblueTransparent() color.RGBA {
	return color.RGBA{R: 0x41, G: 0x69, B: 0xe1, A: 0x00} // rgb(65, 105, 225)
}

func NewSaddlebrownTransparent() color.RGBA {
	return color.RGBA{R: 0x8b, G: 0x45, B: 0x13, A: 0x00} // rgb(139, 69, 19)
}

func NewSalmonTransparent() color.RGBA {
	return color.RGBA{R: 0xfa, G: 0x80, B: 0x72, A: 0x00} // rgb(250, 128, 114)
}

func NewSandybrownTransparent() color.RGBA {
	return color.RGBA{R: 0xf4, G: 0xa4, B: 0x60, A: 0x00} // rgb(244, 164, 96)
}

func NewSeagreenTransparent() color.RGBA {
	return color.RGBA{R: 0x2e, G: 0x8b, B: 0x57, A: 0x00} // rgb(46, 139, 87)
}

func NewSeashellTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xf5, B: 0xee, A: 0x00} // rgb(255, 245, 238)
}

func NewSiennaTransparent() color.RGBA {
	return color.RGBA{R: 0xa0, G: 0x52, B: 0x2d, A: 0x00} // rgb(160, 82, 45)
}

func NewSilverTransparent() color.RGBA {
	return color.RGBA{R: 0xc0, G: 0xc0, B: 0xc0, A: 0x00} // rgb(192, 192, 192)
}

func NewSkyblueTransparent() color.RGBA {
	return color.RGBA{R: 0x87, G: 0xce, B: 0xeb, A: 0x00} // rgb(135, 206, 235)
}

func NewSlateblueTransparent() color.RGBA {
	return color.RGBA{R: 0x6a, G: 0x5a, B: 0xcd, A: 0x00} // rgb(106, 90, 205)
}

func NewSlategrayTransparent() color.RGBA {
	return color.RGBA{R: 0x70, G: 0x80, B: 0x90, A: 0x00} // rgb(112, 128, 144)
}

func NewSlategreyTransparent() color.RGBA {
	return color.RGBA{R: 0x70, G: 0x80, B: 0x90, A: 0x00} // rgb(112, 128, 144)
}

func NewSnowTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xfa, B: 0xfa, A: 0x00} // rgb(255, 250, 250)
}

func NewSpringgreenTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0xff, B: 0x7f, A: 0x00} // rgb(0, 255, 127)
}

func NewSteelblueTransparent() color.RGBA {
	return color.RGBA{R: 0x46, G: 0x82, B: 0xb4, A: 0x00} // rgb(70, 130, 180)
}

func NewTanTransparent() color.RGBA {
	return color.RGBA{R: 0xd2, G: 0xb4, B: 0x8c, A: 0x00} // rgb(210, 180, 140)
}

func NewTealTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0x80, B: 0x80, A: 0x00} // rgb(0, 128, 128)
}

func NewThistleTransparent() color.RGBA {
	return color.RGBA{R: 0xd8, G: 0xbf, B: 0xd8, A: 0x00} // rgb(216, 191, 216)
}

func NewTomatoTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0x63, B: 0x47, A: 0x00} // rgb(255, 99, 71)
}

func NewTurquoiseTransparent() color.RGBA {
	return color.RGBA{R: 0x40, G: 0xe0, B: 0xd0, A: 0x00} // rgb(64, 224, 208)
}

func NewVioletTransparent() color.RGBA {
	return color.RGBA{R: 0xee, G: 0x82, B: 0xee, A: 0x00} // rgb(238, 130, 238)
}

func NewWheatTransparent() color.RGBA {
	return color.RGBA{R: 0xf5, G: 0xde, B: 0xb3, A: 0x00} // rgb(245, 222, 179)
}

func NewWhiteTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xff, B: 0xff, A: 0x00} // rgb(255, 255, 255)
}

func NewWhitesmokeTransparent() color.RGBA {
	return color.RGBA{R: 0xf5, G: 0xf5, B: 0xf5, A: 0x00} // rgb(245, 245, 245)
}

func NewYellowTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xff, B: 0x00, A: 0x00} // rgb(255, 255, 0)
}

func NewYellowgreenTransparent() color.RGBA {
	return color.RGBA{R: 0x9a, G: 0xcd, B: 0x32, A: 0x00} // rgb(154, 205, 50)
}

func NewAliceblueHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xf0, G: 0xf8, B: 0xff, A: 0x80} // rgb(240, 248, 255)
}

func NewAntiquewhiteHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xfa, G: 0xeb, B: 0xd7, A: 0x80} // rgb(250, 235, 215)
}

func NewAquaHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0xff, B: 0xff, A: 0x80} // rgb(0, 255, 255)
}

func NewAquamarineHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x7f, G: 0xff, B: 0xd4, A: 0x80} // rgb(127, 255, 212)
}

func NewAzureHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xf0, G: 0xff, B: 0xff, A: 0x80} // rgb(240, 255, 255)
}

func NewBeigeHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xf5, G: 0xf5, B: 0xdc, A: 0x80} // rgb(245, 245, 220)
}

func NewBisqueHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xe4, B: 0xc4, A: 0x80} // rgb(255, 228, 196)
}

func NewBlackHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0x00, B: 0x00, A: 0x80} // rgb(0, 0, 0)
}

func NewBlanchedalmondHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xeb, B: 0xcd, A: 0x80} // rgb(255, 235, 205)
}

func NewBlueHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0x00, B: 0xff, A: 0x80} // rgb(0, 0, 255)
}

func NewBluevioletHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x8a, G: 0x2b, B: 0xe2, A: 0x80} // rgb(138, 43, 226)
}

func NewBrownHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xa5, G: 0x2a, B: 0x2a, A: 0x80} // rgb(165, 42, 42)
}

func NewBurlywoodHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xde, G: 0xb8, B: 0x87, A: 0x80} // rgb(222, 184, 135)
}

func NewCadetblueHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x5f, G: 0x9e, B: 0xa0, A: 0x80} // rgb(95, 158, 160)
}

func NewChartreuseHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x7f, G: 0xff, B: 0x00, A: 0x80} // rgb(127, 255, 0)
}

func NewChocolateHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xd2, G: 0x69, B: 0x1e, A: 0x80} // rgb(210, 105, 30)
}

func NewCoralHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0x7f, B: 0x50, A: 0x80} // rgb(255, 127, 80)
}

func NewCornflowerblueHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x64, G: 0x95, B: 0xed, A: 0x80} // rgb(100, 149, 237)
}

func NewCornsilkHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xf8, B: 0xdc, A: 0x80} // rgb(255, 248, 220)
}

func NewCrimsonHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xdc, G: 0x14, B: 0x3c, A: 0x80} // rgb(220, 20, 60)
}

func NewCyanHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0xff, B: 0xff, A: 0x80} // rgb(0, 255, 255)
}

func NewDarkblueHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0x00, B: 0x8b, A: 0x80} // rgb(0, 0, 139)
}

func NewDarkcyanHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0x8b, B: 0x8b, A: 0x80} // rgb(0, 139, 139)
}

func NewDarkgoldenrodHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xb8, G: 0x86, B: 0x0b, A: 0x80} // rgb(184, 134, 11)
}

func NewDarkgrayHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xa9, G: 0xa9, B: 0xa9, A: 0x80} // rgb(169, 169, 169)
}

func NewDarkgreenHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0x64, B: 0x00, A: 0x80} // rgb(0, 100, 0)
}

func NewDarkgreyHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xa9, G: 0xa9, B: 0xa9, A: 0x80} // rgb(169, 169, 169)
}

func NewDarkkhakiHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xbd, G: 0xb7, B: 0x6b, A: 0x80} // rgb(189, 183, 107)
}

func NewDarkmagentaHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x8b, G: 0x00, B: 0x8b, A: 0x80} // rgb(139, 0, 139)
}

func NewDarkolivegreenHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x55, G: 0x6b, B: 0x2f, A: 0x80} // rgb(85, 107, 47)
}

func NewDarkorangeHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0x8c, B: 0x00, A: 0x80} // rgb(255, 140, 0)
}

func NewDarkorchidHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x99, G: 0x32, B: 0xcc, A: 0x80} // rgb(153, 50, 204)
}

func NewDarkredHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x8b, G: 0x00, B: 0x00, A: 0x80} // rgb(139, 0, 0)
}

func NewDarksalmonHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xe9, G: 0x96, B: 0x7a, A: 0x80} // rgb(233, 150, 122)
}

func NewDarkseagreenHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x8f, G: 0xbc, B: 0x8f, A: 0x80} // rgb(143, 188, 143)
}

func NewDarkslateblueHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x48, G: 0x3d, B: 0x8b, A: 0x80} // rgb(72, 61, 139)
}

func NewDarkslategrayHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x2f, G: 0x4f, B: 0x4f, A: 0x80} // rgb(47, 79, 79)
}

func NewDarkslategreyHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x2f, G: 0x4f, B: 0x4f, A: 0x80} // rgb(47, 79, 79)
}

func NewDarkturquoiseHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0xce, B: 0xd1, A: 0x80} // rgb(0, 206, 209)
}

func NewDarkvioletHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x94, G: 0x00, B: 0xd3, A: 0x80} // rgb(148, 0, 211)
}

func NewDeeppinkHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0x14, B: 0x93, A: 0x80} // rgb(255, 20, 147)
}

func NewDeepskyblueHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0xbf, B: 0xff, A: 0x80} // rgb(0, 191, 255)
}

func NewDimgrayHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x69, G: 0x69, B: 0x69, A: 0x80} // rgb(105, 105, 105)
}

func NewDimgreyHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x69, G: 0x69, B: 0x69, A: 0x80} // rgb(105, 105, 105)
}

func NewDodgerblueHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x1e, G: 0x90, B: 0xff, A: 0x80} // rgb(30, 144, 255)
}

func NewFirebrickHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xb2, G: 0x22, B: 0x22, A: 0x80} // rgb(178, 34, 34)
}

func NewFloralwhiteHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xfa, B: 0xf0, A: 0x80} // rgb(255, 250, 240)
}

func NewForestgreenHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x22, G: 0x8b, B: 0x22, A: 0x80} // rgb(34, 139, 34)
}

func NewFuchsiaHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0x00, B: 0xff, A: 0x80} // rgb(255, 0, 255)
}

func NewGainsboroHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xdc, G: 0xdc, B: 0xdc, A: 0x80} // rgb(220, 220, 220)
}

func NewGhostwhiteHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xf8, G: 0xf8, B: 0xff, A: 0x80} // rgb(248, 248, 255)
}

func NewGoldHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xd7, B: 0x00, A: 0x80} // rgb(255, 215, 0)
}

func NewGoldenrodHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xda, G: 0xa5, B: 0x20, A: 0x80} // rgb(218, 165, 32)
}

func NewGrayHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x80, G: 0x80, B: 0x80, A: 0x80} // rgb(128, 128, 128)
}

func NewGreenHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0x80, B: 0x00, A: 0x80} // rgb(0, 128, 0)
}

func NewGreenyellowHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xad, G: 0xff, B: 0x2f, A: 0x80} // rgb(173, 255, 47)
}

func NewGreyHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x80, G: 0x80, B: 0x80, A: 0x80} // rgb(128, 128, 128)
}

func NewHoneydewHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xf0, G: 0xff, B: 0xf0, A: 0x80} // rgb(240, 255, 240)
}

func NewHotpinkHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0x69, B: 0xb4, A: 0x80} // rgb(255, 105, 180)
}

func NewIndianredHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xcd, G: 0x5c, B: 0x5c, A: 0x80} // rgb(205, 92, 92)
}

func NewIndigoHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x4b, G: 0x00, B: 0x82, A: 0x80} // rgb(75, 0, 130)
}

func NewIvoryHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xff, B: 0xf0, A: 0x80} // rgb(255, 255, 240)
}

func NewKhakiHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xf0, G: 0xe6, B: 0x8c, A: 0x80} // rgb(240, 230, 140)
}

func NewLavenderHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xe6, G: 0xe6, B: 0xfa, A: 0x80} // rgb(230, 230, 250)
}

func NewLavenderblushHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xf0, B: 0xf5, A: 0x80} // rgb(255, 240, 245)
}

func NewLawngreenHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x7c, G: 0xfc, B: 0x00, A: 0x80} // rgb(124, 252, 0)
}

func NewLemonchiffonHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xfa, B: 0xcd, A: 0x80} // rgb(255, 250, 205)
}

func NewLightblueHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xad, G: 0xd8, B: 0xe6, A: 0x80} // rgb(173, 216, 230)
}

func NewLightcoralHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xf0, G: 0x80, B: 0x80, A: 0x80} // rgb(240, 128, 128)
}

func NewLightcyanHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xe0, G: 0xff, B: 0xff, A: 0x80} // rgb(224, 255, 255)
}

func NewLightgoldenrodyellowHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xfa, G: 0xfa, B: 0xd2, A: 0x80} // rgb(250, 250, 210)
}

func NewLightgrayHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xd3, G: 0xd3, B: 0xd3, A: 0x80} // rgb(211, 211, 211)
}

func NewLightgreenHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x90, G: 0xee, B: 0x90, A: 0x80} // rgb(144, 238, 144)
}

func NewLightgreyHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xd3, G: 0xd3, B: 0xd3, A: 0x80} // rgb(211, 211, 211)
}

func NewLightpinkHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xb6, B: 0xc1, A: 0x80} // rgb(255, 182, 193)
}

func NewLightsalmonHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xa0, B: 0x7a, A: 0x80} // rgb(255, 160, 122)
}

func NewLightseagreenHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x20, G: 0xb2, B: 0xaa, A: 0x80} // rgb(32, 178, 170)
}

func NewLightskyblueHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x87, G: 0xce, B: 0xfa, A: 0x80} // rgb(135, 206, 250)
}

func NewLightslategrayHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x77, G: 0x88, B: 0x99, A: 0x80} // rgb(119, 136, 153)
}

func NewLightslategreyHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x77, G: 0x88, B: 0x99, A: 0x80} // rgb(119, 136, 153)
}

func NewLightsteelblueHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xb0, G: 0xc4, B: 0xde, A: 0x80} // rgb(176, 196, 222)
}

func NewLightyellowHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xff, B: 0xe0, A: 0x80} // rgb(255, 255, 224)
}

func NewLimeHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0xff, B: 0x00, A: 0x80} // rgb(0, 255, 0)
}

func NewLimegreenHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x32, G: 0xcd, B: 0x32, A: 0x80} // rgb(50, 205, 50)
}

func NewLinenHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xfa, G: 0xf0, B: 0xe6, A: 0x80} // rgb(250, 240, 230)
}

func NewMagentaHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0x00, B: 0xff, A: 0x80} // rgb(255, 0, 255)
}

func NewMaroonHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x80, G: 0x00, B: 0x00, A: 0x80} // rgb(128, 0, 0)
}

func NewMediumaquamarineHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x66, G: 0xcd, B: 0xaa, A: 0x80} // rgb(102, 205, 170)
}

func NewMediumblueHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0x00, B: 0xcd, A: 0x80} // rgb(0, 0, 205)
}

func NewMediumorchidHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xba, G: 0x55, B: 0xd3, A: 0x80} // rgb(186, 85, 211)
}

func NewMediumpurpleHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x93, G: 0x70, B: 0xdb, A: 0x80} // rgb(147, 112, 219)
}

func NewMediumseagreenHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x3c, G: 0xb3, B: 0x71, A: 0x80} // rgb(60, 179, 113)
}

func NewMediumslateblueHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x7b, G: 0x68, B: 0xee, A: 0x80} // rgb(123, 104, 238)
}

func NewMediumspringgreenHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0xfa, B: 0x9a, A: 0x80} // rgb(0, 250, 154)
}

func NewMediumturquoiseHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x48, G: 0xd1, B: 0xcc, A: 0x80} // rgb(72, 209, 204)
}

func NewMediumvioletredHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xc7, G: 0x15, B: 0x85, A: 0x80} // rgb(199, 21, 133)
}

func NewMidnightblueHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x19, G: 0x19, B: 0x70, A: 0x80} // rgb(25, 25, 112)
}

func NewMintcreamHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xf5, G: 0xff, B: 0xfa, A: 0x80} // rgb(245, 255, 250)
}

func NewMistyroseHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xe4, B: 0xe1, A: 0x80} // rgb(255, 228, 225)
}

func NewMoccasinHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xe4, B: 0xb5, A: 0x80} // rgb(255, 228, 181)
}

func NewNavajowhiteHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xde, B: 0xad, A: 0x80} // rgb(255, 222, 173)
}

func NewNavyHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0x00, B: 0x80, A: 0x80} // rgb(0, 0, 128)
}

func NewOldlaceHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xfd, G: 0xf5, B: 0xe6, A: 0x80} // rgb(253, 245, 230)
}

func NewOliveHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x80, G: 0x80, B: 0x00, A: 0x80} // rgb(128, 128, 0)
}

func NewOlivedrabHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x6b, G: 0x8e, B: 0x23, A: 0x80} // rgb(107, 142, 35)
}

func NewOrangeHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xa5, B: 0x00, A: 0x80} // rgb(255, 165, 0)
}

func NewOrangeredHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0x45, B: 0x00, A: 0x80} // rgb(255, 69, 0)
}

func NewOrchidHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xda, G: 0x70, B: 0xd6, A: 0x80} // rgb(218, 112, 214)
}

func NewPalegoldenrodHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xee, G: 0xe8, B: 0xaa, A: 0x80} // rgb(238, 232, 170)
}

func NewPalegreenHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x98, G: 0xfb, B: 0x98, A: 0x80} // rgb(152, 251, 152)
}

func NewPaleturquoiseHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xaf, G: 0xee, B: 0xee, A: 0x80} // rgb(175, 238, 238)
}

func NewPalevioletredHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xdb, G: 0x70, B: 0x93, A: 0x80} // rgb(219, 112, 147)
}

func NewPapayawhipHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xef, B: 0xd5, A: 0x80} // rgb(255, 239, 213)
}

func NewPeachpuffHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xda, B: 0xb9, A: 0x80} // rgb(255, 218, 185)
}

func NewPeruHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xcd, G: 0x85, B: 0x3f, A: 0x80} // rgb(205, 133, 63)
}

func NewPinkHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xc0, B: 0xcb, A: 0x80} // rgb(255, 192, 203)
}

func NewPlumHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xdd, G: 0xa0, B: 0xdd, A: 0x80} // rgb(221, 160, 221)
}

func NewPowderblueHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xb0, G: 0xe0, B: 0xe6, A: 0x80} // rgb(176, 224, 230)
}

func NewPurpleHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x80, G: 0x00, B: 0x80, A: 0x80} // rgb(128, 0, 128)
}

func NewRedHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0x00, B: 0x00, A: 0x80} // rgb(255, 0, 0)
}

func NewRosybrownHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xbc, G: 0x8f, B: 0x8f, A: 0x80} // rgb(188, 143, 143)
}

func NewRoyalblueHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x41, G: 0x69, B: 0xe1, A: 0x80} // rgb(65, 105, 225)
}

func NewSaddlebrownHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x8b, G: 0x45, B: 0x13, A: 0x80} // rgb(139, 69, 19)
}

func NewSalmonHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xfa, G: 0x80, B: 0x72, A: 0x80} // rgb(250, 128, 114)
}

func NewSandybrownHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xf4, G: 0xa4, B: 0x60, A: 0x80} // rgb(244, 164, 96)
}

func NewSeagreenHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x2e, G: 0x8b, B: 0x57, A: 0x80} // rgb(46, 139, 87)
}

func NewSeashellHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xf5, B: 0xee, A: 0x80} // rgb(255, 245, 238)
}

func NewSiennaHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xa0, G: 0x52, B: 0x2d, A: 0x80} // rgb(160, 82, 45)
}

func NewSilverHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xc0, G: 0xc0, B: 0xc0, A: 0x80} // rgb(192, 192, 192)
}

func NewSkyblueHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x87, G: 0xce, B: 0xeb, A: 0x80} // rgb(135, 206, 235)
}

func NewSlateblueHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x6a, G: 0x5a, B: 0xcd, A: 0x80} // rgb(106, 90, 205)
}

func NewSlategrayHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x70, G: 0x80, B: 0x90, A: 0x80} // rgb(112, 128, 144)
}

func NewSlategreyHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x70, G: 0x80, B: 0x90, A: 0x80} // rgb(112, 128, 144)
}

func NewSnowHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xfa, B: 0xfa, A: 0x80} // rgb(255, 250, 250)
}

func NewSpringgreenHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0xff, B: 0x7f, A: 0x80} // rgb(0, 255, 127)
}

func NewSteelblueHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x46, G: 0x82, B: 0xb4, A: 0x80} // rgb(70, 130, 180)
}

func NewTanHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xd2, G: 0xb4, B: 0x8c, A: 0x80} // rgb(210, 180, 140)
}

func NewTealHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x00, G: 0x80, B: 0x80, A: 0x80} // rgb(0, 128, 128)
}

func NewThistleHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xd8, G: 0xbf, B: 0xd8, A: 0x80} // rgb(216, 191, 216)
}

func NewTomatoHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0x63, B: 0x47, A: 0x80} // rgb(255, 99, 71)
}

func NewTurquoiseHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x40, G: 0xe0, B: 0xd0, A: 0x80} // rgb(64, 224, 208)
}

func NewVioletHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xee, G: 0x82, B: 0xee, A: 0x80} // rgb(238, 130, 238)
}

func NewWheatHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xf5, G: 0xde, B: 0xb3, A: 0x80} // rgb(245, 222, 179)
}

func NewWhiteHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xff, B: 0xff, A: 0x80} // rgb(255, 255, 255)
}

func NewWhitesmokeHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xf5, G: 0xf5, B: 0xf5, A: 0x80} // rgb(245, 245, 245)
}

func NewYellowHalfTransparent() color.RGBA {
	return color.RGBA{R: 0xff, G: 0xff, B: 0x00, A: 0x80} // rgb(255, 255, 0)
}

func NewYellowgreenHalfTransparent() color.RGBA {
	return color.RGBA{R: 0x9a, G: 0xcd, B: 0x32, A: 0x80} // rgb(154, 205, 50)
}
